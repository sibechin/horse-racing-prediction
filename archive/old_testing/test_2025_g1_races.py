"""
2025年G1レースでLightGBMモデルをテスト

訓練データ: 2010-2024年5月 (4,852レコード)
テストデータ: 2025年G1レース (18レース)
"""
import pandas as pd
import numpy as np
...
# 2025年G1レース情報
G1_RACES_2025 = {
    'フェブラリーステークス': {'date': '2025-02-23', 'venue': '中山'},
    '高松宮記念': {'date': '2025-03-30', 'venue': '中京'},
    '桜花賞': {'date': '2025-04-13', 'venue': '阪神'},
    '皐月賞': {'date': '2025-04-20', 'venue': '中山'},
    '天皇賞春': {'date': '2025-05-04', 'venue': '京都'},
    'NHKマイルカップ': {'date': '2025-05-11', 'venue': '東京'},
    'ヴィクトリアマイル': {'date': '2025-05-18', 'venue': '東京'},
    'オークス': {'date': '2025-05-25', 'venue': '東京'},
    '日本ダービー': {'date': '2025-06-01', 'venue': '東京'},
    '安田記念': {'date': '2025-06-08', 'venue': '東京'},
    '宝塚記念': {'date': '2025-06-15', 'venue': '阪神'},
    'スプリンターズステークス': {'date': '2025-09-28', 'venue': '中山'},
    '秋華賞': {'date': '2025-10-19', 'venue': '京都'},
    '菊花賞': {'date': '2025-10-26', 'venue': '京都'},
    '天皇賞秋': {'date': '2025-11-02', 'venue': '東京'},
    'エリザベス女王杯': {'date': '2025-11-16', 'venue': '京都'},
}

class LightGBMTester:
    """LightGBM 7-Foldモデルテスター"""
    ...
    def test_on_2025_data(self, data_path: str):
        ...
        try:
            ...
            logging.info(f"\nデータ読み込み: {len(df)}レコード")
            logging.info(f"データ期間: {df['race_date'].min()} - {df['race_date'].max()}")
        except FileNotFoundError:
            logging.error(f"❌ ファイルが見つかりません: {data_path}")
            logging.info("\n2025年のG1レースデータをnetkeibaから収集してください")
            return None

        # 2025年データのみ抽出
        df['race_date'] = pd.to_datetime(df['race_date'])
        df_2025 = df[df['race_date'].dt.year == 2025].copy()

        if len(df_2025) == 0:
            logging.warning("⚠️ 2025年のデータが見つかりません")
            return None

        logging.info(f"\n2025年データ: {len(df_2025)}レコード, {df_2025['race_id'].nunique()}レース")

        # 各レースで評価
        results = []

        for race_id in df_2025['race_id'].unique():
            race_data = df_2025[df_2025['race_id'] == race_id].copy()
            race_name = race_data['race_id'].iloc[0]
...
def main():
    ...
    # テスター初期化
    tester = LightGBMTester(model_dir='models/optimized')
    ...
    if results:
        ...
        logging.info("\n❌ 2025年のG1レースデータが必要です")
        logging.info("netkeibaから以下のレースを収集してください:")
        for race_name, info in G1_RACES_2025.items():
            logging.info(f"  - {race_name} ({info['date']}, {info['venue']})")
