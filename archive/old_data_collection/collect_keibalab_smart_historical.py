"""
keibalab.jp スマート歴史的G1データ収集

逆年順で収集し、成功率が低下したら自動停止する賢い収集戦略
"""
import sys
...
def collect_year_data(year: int, scraper: KeibalabScraper) -> tuple:
    ...
    logging.info(f"推定所要時間: {len(races) * 12 / 60:.1f}分")
    logging.info("")
...
def collect_smart_historical(start_year: int = 2024, end_year: int = 2000,
                             min_success_rate: float = 0.05, delay: float = 12.0):
    """
    逆年順でスマートに歴史的G1データを収集
    ...
    """
    logging.info("=" * 60)
    logging.info("keibalab.jp スマート歴史的G1データ収集")
    logging.info("=" * 60)
    ...
    if all_results:
        ...
        output_file = output_dir / f'g1_smart_historical_{actual_start_year}_{actual_end_year}_{timestamp}.csv'
...
def main():
    """メイン実行"""
    logging.info("=" * 60)
    logging.info("keibalab.jp スマート歴史的G1データ収集スクリプト")
    logging.info("=" * 60)
    logging.info("")
    logging.info("特徴:")
    logging.info("  - 2024年から逆順で収集 (新→旧)")
    logging.info("  - 成功率が5%未満で自動停止")
    logging.info("  - 効率的なデータ収集")
    logging.info("")
    logging.info("推定:")
    logging.info("  - 最大25年分のデータを収集可能")
    logging.info("  - 実際には10-15年分程度が現実的")
    logging.info("  - 推定所要時間: 2-6時間")
    logging.info("")
    logging.info("注意:")
    ...
    # データ収集
    collect_smart_historical(start_year=2024, end_year=2000, min_success_rate=0.05, delay=12.0)
